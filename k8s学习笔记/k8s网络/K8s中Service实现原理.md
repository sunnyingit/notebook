...
在 K8s中，CoreDNS 是一个开源的、轻量级的 DNS 服务器，它被用作 Kubernetes 集群中的默认 DNS 服务。
CoreDNS 帮助 Kubernetes 集群中的各个组件进行服务发现和网络通信，并能够解析 Kubernetes 集群内部的 DNS 名称。
...
## Service常用命令
Service配置虚拟ip时，Kubernetes 集群会自动为每个 Service 分配一个*唯一的IP地址*，并在需要时动态更新它。
Service ip只能在集群中访问，无法被外部访问，这个很好理解，因为Service ip并不是一个真实的ip，只是一个用于网络转发的ip而已。
